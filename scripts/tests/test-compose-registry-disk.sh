...
check(len(COMPOSE_REGISTRY) == 105, f"registry expects 105 entries (got {len(COMPOSE_REGISTRY)})")
check(len(disk_paths) == 118, f"disk expects 118 compose files (got {len(disk_paths)})")
