...
Combined `.env` for vLLM single-card WSL2 — drop it in the **same directory as the compose you launch** (the nested `<topology>/<quant>/` dir, e.g. `models/qwen3.6-27b/vllm/compose/single/autoround-int4/.env`), **not** the parent `compose/.env`. Docker Compose only auto-loads `.env` from the directory it is invoked in, and `switch.sh` / `launch.sh` `cd` into the compose's own dir before `docker compose up` — so a `.env` one level up (e.g. `compose/.env`) is **silently never read** (no error; values just fall back to the compose defaults). Since 2026-08-10 `setup.sh`'s WSL2 auto-detect writes the `PYTORCH_CUDA_ALLOC_CONF` line into each compose directory for you (it used to write the parent path, and now retires that dead file); the other two knobs are yours to add:
