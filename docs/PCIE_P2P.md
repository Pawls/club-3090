...
> **Large BAR1 is necessary, not sufficient — the driver also has to *use* it** ([#873](https://github.com/noonghunna/club-3090/issues/873)). The patch depends on the driver selecting the static full-VRAM BAR1 mapping; nothing guarantees it picks that path on your card and driver branch. On a 2× RTX 5090 rig (@paulp83) the driver advertised peer access (`topo -p2p r` → `OK`) while every vLLM boot hung inside NCCL init. What fixed it was forcing the mapping and the peer-mapping override through the kernel module:
>
> ```
...
>
> ⚠️ **That is a reported-working set, not a minimal one** — the five keys were applied together and which ones are load-bearing is unbisected. `RMForceStaticBar1=1` is the one that names the mechanism above, and `PeerMappingOverride=1` lifts the peer-mapping restriction; note aikitoria's README documents only `RMForceP2PType=1`, and there as a *testing* knob to make 3090s prefer PCIe over NVLink. Try the narrow pair first. Undocumented RM registry keys are a support-free surface — if a boot goes bad, delete the file and rebuild the initramfs.
...
| Second GPU trains at **x8** or disappears | A populated M.2 / adjacent slot is stealing its lanes → move the card or clear the bifurcation jumper (board manual). Launching a TP≥2 compose now auto-warns on this via `switch.sh`'s `[preflight]` lane-width check → [HARDWARE.md](HARDWARE.md#note-for-older-host-platforms-pcie-gen-3--older-cpus). |
...
| `[nvlink] WARNING: … BAR1 is far smaller than VRAM …` | The launcher's BAR1 sanity check fired: P2P is being enabled, but the aperture is too small to back the patch's static full-VRAM mapping (§5). It **warns rather than gating** — the config is applied unchanged, so if this boot then hangs at `pynccl`, that's the row below. If P2P demonstrably works on your rig despite the warning, say so in an issue and bring `nvidia-smi -q -d MEMORY` — that's the case that would turn the warning into a gate. |
...
| **vLLM slugs HANG at `pynccl` init after installing a patched driver/module** (last line `vLLM is using nccl==…`, weights never load, no error) | The driver now *grants* P2P, so `detect_nvlink.sh` auto-enabled the P2P path (§5) — but the grant doesn't carry actual transfers, so NCCL blocks on its first peer op. **Unblock: `NVLINK_MODE=force_off` in `.env`, relaunch** (back to pre-patch behavior). Then validate the grant with raw transfers: cuda-samples `p2pBandwidthLatencyTest`, or §7's `VLLM_SKIP_P2P_CHECK=0` transfer check. If raw P2P hangs/reads garbage, in order: (1) check `nvidia-smi -q -d MEMORY` — a BAR1 far under VRAM means the patch's static mapping isn't in place, so **force it** with the registry override in §5 (this is what resolved the 5090 case, [#873](https://github.com/noonghunna/club-3090/issues/873)); (2) match the patch branch to your **exact** driver version; (3) check ACS (`lspci -vvv \| grep ACSCtl` — ACS redirect stalls peer TLPs) and confirm `iommu=pt` per §4. Common right after a driver upgrade: the patch fork lags new driver branches. |
