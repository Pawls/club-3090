...
declare -A SHORTCUTS=(
  ...
  # apex vision — ik is the one to use (keeps ik's MoE speed). The mainline build is
  # kept only "just in case" and is explicitly named. No bare `apex-vision` alias so
  # there's exactly one name per compose.
