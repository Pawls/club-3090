    ...
    def serve_override_defaults(self, profile_like: str, repo: str) -> dict[str, str]:
        ...
        try:
            ...
            if entry:
                ...
                if sm:
                    method = sm.group(1)
                    # num_speculative_tokens is written three ways across the
                    # estate, and each of these was a real miss at some point:
                    #   "num_speculative_tokens":3            literal
                    #   "num_speculative_tokens":${SPEC_N:-4} parameterized default
                    #   \"num_speculative_tokens\":3          escaped, inside an
                    #                                         entrypoint-built shell
                    #                                         heredoc
                    # So: tolerate the escaped quotes (symmetric with the "method"
                    # regex above), then optionally consume a ${VAR:- prefix under
                    # ANY var name — 13 composes parameterize this under five names
                    # (SPEC_N, SPEC_N_MAX, NUM_SPEC_TOKENS, MTP_SPEC_TOKENS, …) —
                    # and read the DEFAULT, which is what the compose serves unless
                    # the operator overrides it, i.e. exactly what this editor
                    ...
                    if not nm:
                        # Entrypoint-built configs interpolate a $$_spec_n shell var
                        # at the key, so the key and the number are not adjacent —
                        # the real default lives in a detached ${SPEC_N:-<n>}.
