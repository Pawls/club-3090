...
| `qwen3.6-40b-deckard/llama-cpp/…/dual/piehsoft-q6k/` | `CUDA_VISIBLE_DEVICES=0,1` · `TENSOR_SPLIT=1.1,1.0` (mtp.yml only — vision.yml uses its own even-split var) · `CTX_SIZE=131072` · `KV_TYPE=q8_0` · `NP=1` · `MTP_DRAFT_N_MAX=2` · `REASONING=off` · `PRESERVE_THINKING=false` (both 2026-08-07: were `on`/`true`; back to the upstream compose defaults) · `PORT=8199` |
...
| `qwen3.6-27b/vllm/…/dual/autoround-int4/` | `GPU_MEMORY_UTILIZATION=0.86` (desktop headroom) · `ENABLE_THINKING=false` · `PRESERVE_THINKING=false` (both flipped from `true` 2026-08-07 — see BENCHMARKS.md "Thinking on vs off" + [disc #221](https://github.com/noonghunna/club-3090/discussions/221)) (alloc-conf + `MTP_SPEC_TOKENS` commented out) |
