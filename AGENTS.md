...
#### `.env` overrides must live next to the compose file, not the engine root

Docker Compose only auto-loads a `.env` from the **directory it's invoked from** — it does not search parent directories. `switch.sh` / `launch.sh` `cd` into the compose file's own directory (`<engine>/compose/<topology>/<quant>/`) before running `docker compose up`, so a `.env` sitting at a parent level (e.g. `<engine>/compose/.env`) is silently never read. Overrides in it fall back to the compose's own `${VAR:-default}` values — no error, no warning, and `GPU_MEMORY_UTILIZATION`-style values that happen to match the compose default look like they're "working" by coincidence.

**This bit us for months (2026-07-03 → 2026-08-10):** `scripts/setup.sh`'s WSL2 auto-detection (`PYTORCH_CUDA_ALLOC_CONF` boot-crash workaround, PR #84 / issue #60) wrote its override to `<engine>/compose/.env` — one level too high for any nested `<topology>/<quant>/` compose to see. Reproduced on `models/qwen3.6-27b/vllm/compose/dual/autoround-int4/fp8-mtp.yml`: `docker compose config` (run from the compose's own directory) showed the compose default, not the `.env` override, until the `.env` was copied down into `.../dual/autoround-int4/.env`.

**Fixed at the source (2026-08-10).** `scripts/lib/wsl2-env.sh` (`wsl2_sync_alloc_conf`, called from `setup.sh`) writes one `.env` per **quant directory** — the level compose reads — and retires the dead compose-root file. Three properties worth preserving if you touch it:

- **The value is derived per directory**, by flipping only the `expandable_segments` token in that directory's own `${PYTORCH_CUDA_ALLOC_CONF:-…}` default. Other knobs (`max_split_size_mb`, …) survive, and a directory that doesn't opt into expandable_segments (vllm-lmcache omits it deliberately) gets no file. Same principle as `detect_nvlink.sh`'s alloc-conf strip.
- **Existing `.env` files are never rewritten** — they're user files. An active assignment is left alone; a commented-out one is treated as a deliberate opt-out; a file with no mention gets a warning naming the exact line to add.
- **A hardcoded `PYTORCH_CUDA_ALLOC_CONF=<v>` in a compose defeats all of this** — no `.env` can override a non-interpolated value. Four Gemma composes had one (including the `gemma-4-31b` dual default) and were parameterized in the same change.

`scripts/tests/test-wsl2-env-placement.sh` guards all of it, plus two repo-wide invariants: no live vLLM compose may hardcode the var, and no `.env` may sit at the never-read compose-root level. **The `--env-file` alternative was rejected**: pointing the launchers at the engine-root `.env` would make every stale parent file across the estate suddenly live, which is a behavior change users never asked for.
...
- **⚠️ FIRST — ASK WHICH REGISTRY, and never write the curated catalog on a user's checkout.** Three destinations, not interchangeable. c3's Bring & Validate lane already states them verbatim — reuse its wording: `⏎ Write LOCAL layer · C WRITE CORE REGISTRY (needs C3_ALLOW_CORE_PROMOTE=1) · E Export as PR bundle`.
  ...
  - **Maintainer, on the canonical rig** — `promote.py --layer core`, gated on **both** `--layer core` and `C3_ALLOW_CORE_PROMOTE=1`. That gate is a plain env var and **`.env` is not read by these tools** (no shell wrapper sources it) — `export` it or pass it per-invocation. **The curated-catalog steps below are this row only.**
